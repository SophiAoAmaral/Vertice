  ...
  return (
    <div className='container'>
        ...
        {itensFiltrados.map((dado)=>(
            <div key={dado.id} className='flex flex-col items-center  w-150'>
                <Link to={`/produto/${dado._id}`} className='inline-block'>
                    <img src={dado.image} alt={dado.nome} className='h-100 w-full object-contain mb-5' />
                    <div className='flex justify-between'>
                        <h3>{dado.nome}</h3>
                        <span>R$ {dado.preco}</span>
                    </div>
                    <div className='flex justify-center flex-col-reverse mt-2 items-center gap-2'>
                        <button className='border rounded-2xl py-3 px-5'>Adicionar ao carrinho</button>
                        <button className='rounded-2xl py-3 px-5 bg-azul text-white hover:bg-azul-hover'>Comprar</button>
                    </div>
                </Link>
            </div>
        ))}
